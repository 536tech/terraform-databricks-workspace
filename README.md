...
The root module is a composition module. It reads 15 input maps and creates one child module
...
| `catalog_access` | `map(map(list(string)))` | Direct grants ON a catalog. Shape: catalog name -> principal -> [privileges]. Principals: groups/users by name or email; service principals by application id. | `{}` | no |
